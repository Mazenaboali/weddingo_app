...
import 'package:weddingo/My_database.dart';
          ...
          children: [
            ...
            Expanded(
                child: StreamBuilder<QuerySnapshot<Hall>>(
                    stream: MyDataBase.getHallsRealTieUpdates(governortecomparedondatabase),
                    builder: (buildContext, snapshot) {
                      ...
                      var halls =snapshot.data?.docs.map((doc) => doc.data()).toList();
                      if (snapshot.hasError) {
                        return Center(child: Text('Error loading halls.'));
                      }
                      return halls?.length==0 ?Center(child: Text('There is no halls available in this governorate ')):ListView.builder(
                        itemBuilder: (_, index) {
                          return InkWell(
                            onTap: (){
                              Navigator.pushNamed(context, HallDetailsScreen.routeName);
